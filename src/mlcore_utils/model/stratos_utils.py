...
from mlcore_utils.model.data import (
    ...
    Stratos_Deployer_V1_Data_Interface,
)
...
class Stratos_Util(object):
    ...
    def build_container(
        self, containerbuild_data: Stratos_ContainerBuild_Metadata_V1
    ) -> Result[str, str]:
        ...
        if response.status_code == 200:
            ...
            result = self.stratos_api_caller.call_status_url_and_await(
                status_response_url
            )
            if is_ok(result):
                if result.ok_value.status_code == 200:
                    ...
                    return Err(
                        f"Building container image failed with error {result.ok_value.json()['msg']}"
                    )
                else:
                    return Err(
                        f"Building container image failed with error {result.ok_value.text}"
                    )
            elif is_err(result):
        ...
        if is_ok(argocd_proeject_result):
            if argocd_proeject_result.ok_value:
                ...
                stratos_application_metadata = (
                    deployer_data.get_stratos_appownersmetadata_v1(
                        deployer_data.get_stratos_application_name()
                    )
                )
                ...
                if is_ok(stratos_application_result):
                    ...
                    if helm_data.base64_values_yaml_contents:
                        deploy_result = self.util.deploy_helm_chart_and_values(
                            helm_data
                        )
                    else:
                    ...
                    if is_ok(deploy_result):
                        ...
                        if helm_deploy_success:
                            ...
                            app_sync_request = (
                                deployer_data.get_stratos_appsyncargorequest_v1()
                            )
